...
export default function Ideation() {
  ...
  const filteredIdeas = activeFilters.length > 0 ? ideas.filter(idea => idea.tags.some(tag => activeFilters.includes(tag))) : ideas;
  return <div className="min-h-screen bg-gradient-to-br from-slate-50/90 to-slate-100/80">
        ...
        <div className="w-64 min-h-screen pt-20 px-4 border-r border-slate-200/50 bg-white/40 backdrop-blur-xl">
          <div className="space-y-4">
            <div>
              ...
              <Button variant="ghost" onClick={() => setIsFilterOpen(true)} className="w-full bg-white/50 hover:bg-white/60 justify-start font-extralight text-left px-0">
                <Filter className="h-4 w-4 mr-2" />
                Filter Ideas
              </Button>
            </div>
            {activeFilters.length > 0 && <div className="space-y-2">
                <p className="text-sm font-medium text-slate-600">Active Filters:</p>
                <div className="flex flex-wrap gap-2">
                  {activeFilters.map(filter => <span key={filter} className="px-2 py-1 text-xs rounded-full bg-purple-100 text-purple-700">
                      {filter}
                    </span>)}
                </div>
              </div>}
        ...
        <main className="flex-1 pt-20 px-4 md:px-8">
          <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredIdeas.map(idea => <IdeaCard key={idea.id} idea={idea} onClick={() => setSelectedIdea(idea)} />)}
            </div>
          </div>
        </main>
      </div>
      ...
      {/* Modals */}
      <FilterDialog open={isFilterOpen} onOpenChange={setIsFilterOpen} activeFilters={activeFilters} setActiveFilters={setActiveFilters} />
